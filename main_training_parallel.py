...
def train(model, device, train_loader, criterion, optimizer, epoch, args):
    ...
    for i, (images, labels) in enumerate(train_loader):
        if args.dynamic!=0:
            model.module.T= torch.randint(1,args.T+1,(1,)).item()
            #model.module.T= torch.randint(args.T-1,args.T+2,(1,)).item()
            #model.module.T= torch.randint(args.T-1,args.T+1,(1,)).item()
        optimizer.zero_grad()
...
if __name__ == '__main__':
    ...
    if args.dataset == 'cifar10':
        ...
        train_dataset, val_dataset = data_loaders.build_dvscifar('/datasets/dvs-cifar10')
        num_classes = 10
    ...
    best_acc = 0
    folder_name =f'outputs/{args.dataset}/{args.dynamic}{args.TET}{args.cut}/' 
    os.makedirs(folder_name, exist_ok=True)
    model_name = folder_name+f'{args.dataset}_resnet19_T{args.T}_dyn{args.dynamic}_TET{args.TET}_seed{args.seed}_cut{args.cut}_batch{args.batch_size}'
